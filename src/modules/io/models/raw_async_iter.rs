...
impl<T> RawAsyncIter<T>
where
    T: AsyncRead + Unpin,
{
    async fn inner_next(&mut self) -> Option<Result<serde_json::Value, LogIOError>> {
...
impl<T> From<T> for RawAsyncIter<T>
where
    T: AsyncRead + Unpin,
{
...
impl<A> From<A> for RawAsyncIter<tokio_util::compat::Compat<A>>
where
    A: tokio::io::AsyncRead + Unpin,
{
    fn from(value: A) -> Self {
        let compat = tokio_util::compat::TokioAsyncReadCompatExt::compat(value);
        RawAsyncIter::from(compat)
    }
}

impl<T> Stream for RawAsyncIter<T>
where
    T: AsyncRead + Unpin,
{

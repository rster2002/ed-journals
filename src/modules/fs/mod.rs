...
//! [SyncBlocker] or [AsyncBlocker]. These can be used to block execution until a watcher unblocks
//! them.
//!
//! Watchers are used to keep track of file system changes, either for the whole log directory using
//! the [DirWatcher] or a single file using the [FileWatcher]. These require a blocker to be passed
//! when constructing them (it doesn't matter which blocker is passed, watchers don't care if you're
//! running sync or async code) and when something changes, they signal to unblock.
//!
...
//! iterators available in the [io](crate::io) module.
//!

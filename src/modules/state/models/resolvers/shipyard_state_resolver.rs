...
use crate::state::models::resolvers::shipyard_state_resolver::ship_entry::{ShipEntry, ShipStatus, TransferringShipStatus};
...
impl StateResolver<LogEvent> for ShipyardStateResolver {
    fn feed(&mut self, input: &LogEvent) -> FeedResult {
        ...
        match &input.content {
            LogEventContent::ShipyardBuy(buy_event) => {
                match &buy_event.old_ship_action {
                    ShipyardBuyEventOldShipAction::Store(store_action) => {
                        let ship_entry = self.track_ship(store_action.store_ship_id, store_action.store_old_ship.clone());
            ...
            LogEventContent::ShipyardSwap(swap_event) => {
                ...
                let old_ship = self.track_ship(swap_event.store_ship_id, swap_event.store_old_ship.clone());

                old_ship.status = ShipStatus::Stored(swap_event.market_id);
            },
            LogEventContent::ShipyardTransfer(transfer_event) => {
                let ship = self.track_ship(transfer_event.ship_id, transfer_event.ship_type.clone());
            ...
            LogEventContent::StoredShips(stored_ships) => {
                for local_ship in &stored_ships.ships_here {
                    let ship_entry = self.track_ship(local_ship.ship_id, local_ship.ship_type.clone());

                    ship_entry.status = ShipStatus::Stored(stored_ships.market_id);
                }

                for remote_ship in &stored_ships.ships_remote {
                    let ship_entry = self.track_ship(remote_ship.ship_id, remote_ship.ship_type.clone());
...
impl ShipyardStateResolver {
    fn track_ship(&mut self, ship_id: u64, ship_type: ShipType) -> &mut ShipEntry {
        self.ships.entry(ship_id)
            .or_insert_with(|| ShipEntry {
                ship_type,
